...
- **Gates obrigatórios (bloqueiam merge)**: os dois jobs acima (guard sempre; no segundo, `flutter analyze` — erros e warnings, infos não bloqueiam —, `flutter test`, `flutter build web` e `flutter build apk`)
- **Atenção — bloqueio permanente até existir teste real**: `test/widget_test.dart` hoje é só o stub vazio gerado pelo `flutter create` — 0 testes reais, `flutter test` sempre retorna "No tests found" (exit 79). Decisão de 2026-08-16: `test` é bloqueante mesmo assim — na prática, **nenhum PR mergeia neste repo até existir pelo menos 1 teste de verdade**.
